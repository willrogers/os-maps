...
class OSNamesService {
    ...
    suspend fun search(query: String): List<SearchResult> =
        if (osKeyIsSet()) searchOS(query) else searchNominatim(query)

    // -------------------------------------------------------------------------
    // OS Names API
    // -------------------------------------------------------------------------

    private suspend fun searchOS(query: String): List<SearchResult> = withContext(Dispatchers.IO) {
        val url = "https://api.os.uk/search/names/v1/find" +
    ...
    private data class OsNamesResponse(@SerializedName("results") val results: List<OsResultItem>?)
    private data class OsResultItem(@SerializedName("GAZETTEER_ENTRY") val gazetteerEntry: OsGazetteerEntry?)
